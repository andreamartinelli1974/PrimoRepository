...
%dataBG_byComune_20_all = dataBG_byComune_20_all(1:88,:);
...
beta0 = [80 0.2 cumdead.Var2(end)];

b0_log = lsqcurvefit(modelfun_log,beta0,cumdead.Var1,cumdead.Var2);%(cumdead, modelfun_log, beta0);

FittedLOG = table(datelog,cumdead.Var2,...
                  modelfun_log(b0_log,xdate20(start(end)+lag:end)-xdate20(1)));      
FittedLOG.Properties.VariableNames = {'xdatetime20','CoviD','Fitted_CoviD'};

PreviewLOG = table(xdatetime(diffDate),modelfun_log(b0_log,xdate(diffDate)-xdate20(1)));
PreviewLOG.Properties.VariableNames = {'xdatetime','Fitted_LOG'};

xdatedead = xdatetime(start(end)+lag:end);
Max_dead = table(xdatedead, repmat(round(b0_log(3)),numel(xdatedead),1));
Max_dead.Properties.VariableNames = {'xdatetime','Deads_Forecast'};
...
%% numero morti
f11 = figure();
p21 = semilogy(FittedLOG.xdatetime20,FittedLOG.CoviD,'DisplayName','CoviD');
hold on;
p21.LineStyle = 'none';
p21.Marker = 'o';
p21.MarkerSize = 3;
p21.Color = [0,0,1];
p22 = semilogy(FittedLOG.xdatetime20,FittedLOG.Fitted_CoviD,'DisplayName','Fitted CoviD');
p23 = semilogy(PreviewLOG.xdatetime,PreviewLOG.Fitted_LOG,'DisplayName','Forecast');
p23.Color = [0.4660 0.6740 0.1880];
p23.LineWidth = 2;
p24 = semilogy(Max_dead.xdatetime,Max_dead.Deads_Forecast,'DisplayName','Max Deads Forecast');
p24.LineStyle = '--';
p24.LineWidth = 1;
p24.Color = [0,0,0];
