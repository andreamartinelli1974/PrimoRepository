function [rankMatrixS]= calculateRankMatrixS (sandwichData,cardCompaniesbyDate,rf,mapSort,legendaIcb)
...
for t=1:size(cardCompaniesbyDate,1)
    ...
    for factor=1:size(timeslice,2)-1 %all rf but the ICB code
        ...
        rankMatrixS(startpoint:endpoint,factor)=ranking_i; %matrix of rank positions for each factor and each timeslice
        clear ranking_i
    end
